...
class Rack::Attack
  ...
  throttle('req/ip', limit: 50, period: 1.second) do |req|
    req.ip
  end
  ...
  # Throttle POST/PUT/PATCH/DELETE requests more strictly. These aren't even available on this app.
  throttle('writes/ip', limit: 5, period: 1.minute) do |req|
