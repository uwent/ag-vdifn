  ...
  class LateBlight < DsvPest
    def severities_from_totals(past_week, season_to_date)
      if past_week.count != season_to_date.count
        Rails.logger.error("Potato Late Blight: past week and season mismatch")
        return []
      end

      past_week.zip(season_to_date).map do | pair |
        Rails.logger.error("Potato Late Blight: Latitude mismatch") if pair[0][:lat] != pair[1][:lat]
        Rails.logger.error("Potato Late Blight: Longitude mismatch") if pair[0][:long] != pair[1][:long]
