<script lang="ts">
  ...
  function setUserMinMax(absoluteMin, absoluteMax) {
    userMin = Math.round(absoluteMin + (absoluteMax - absoluteMin) / (severityLevels * 2));
    userMax = Math.round(absoluteMax - (absoluteMax - absoluteMin) / (severityLevels * 2));
...
Map range: {$mapMinMapMax.min} - {$mapMinMapMax.max} DD
