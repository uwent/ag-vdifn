...
import type { Pest, DegreeDayModel } from '@components/common/ts/types';
...
export const selectedAffliction = writable<Partial<Pest>>({});
export const selectedDDModel = writable<Partial<DegreeDayModel>>({});
...
interface AfflictionPanelState {
  currentAffliction: object;
  selectedExtent: string;
  severities: [];
  severityParams: object;
  loaded: boolean;
}
export const diseasePanelState = writable<Partial<AfflictionPanelState>>({
  loaded: false,
});
export const insectPanelState = writable<Partial<AfflictionPanelState>>({
  loaded: false,
});
...
export const customPanelState = writable<Partial<CustomPanelState>>({
  selectedGradient: 1,
  loaded: false,
});
...
export const twoPointGradientState = writable<Partial<GradientState>>({});
export const threePointGradientState = writable<Partial<GradientState>>({});
